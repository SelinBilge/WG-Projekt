    ...
    @IBAction func nextButton(_ sender: Any) {
        ...
        if wgNametext.text != "" && wgPasswordText.text != "" {
            ...
            if wgname == "" {
                ...
                print("CHECK: \(wgNametext.text!)")
                
                // search given wg in database
                db.collection("wgs").whereField("wgname", isEqualTo: wgNametext.text!).getDocuments() { (querySnapshot, err) in
                    if let err = err {
                ...
                print("Test get DodumentID: \(documentID)")
                ...
                if documentID != "" {
                    ...
                    db.collection("wgs").document(documentID).getDocument { (document, error) in
                        ...
                        if error == nil {
                            ...
                            if document != nil && document!.exists {
                                ...
                                print("JoinExsistingWG -> Test get code: \(users)")
                    ...
                    db.collection("wgs").document(documentID).updateData([
                     ...
                     db.collection("users").document(documentID).updateData(["wgname":wgname, "wgpasswort":wgpasswort, "wgkey": documentID]) { (error) in
